...
void main()
{
	...
	if (dist < tbnRange - 50)
	{
		...
		for (int i = 0; i < numMaterials; i++)
		{			
			bumpNormal += (2*(texture(materials[i].normalMap, UVs[i]).rbg) - 1) * blendValuesArray[i];
		}
	...
	for(int i = 0; i < numMaterials; ++i)
	{
        ...
        metallic  += texture(materials[i].metallicMap, UVs[i]).r * blendValuesArray[i];
        roughness += texture(materials[i].roughnessMap, UVs[i]).r * blendValuesArray[i];
        ao        += texture(materials[i].aoMap, UVs[i]).r * blendValuesArray[i];
	}
