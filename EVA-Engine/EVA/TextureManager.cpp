...
namespace EVA
{
	...
	std::shared_ptr<Texture> TextureManager::LoadTexture(const FS::path& path)
	{
		...
		if (data)
		{
			...
			// Save the texture
			GLCall(glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, channels == 3 ? GL_RGB : GL_RGBA, GL_UNSIGNED_BYTE,
				data));
	...
	std::shared_ptr<Texture> TextureManager::LoadTextureCubemap(const FS::path& folderPath, const std::string& fileType)
	{
		...
		for (unsigned int i = 0; i < 6; i++)
		{
			...
			if (data)
			{
				GLCall(glTexImage2D(sideIds[i], 0, GL_RGBA, width, height, 0, channels == 3 ? GL_RGB : GL_RGBA, GL_UNSIGNED_BYTE,
					data));
				stbi_image_free(data);
