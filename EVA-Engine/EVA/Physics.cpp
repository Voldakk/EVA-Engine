...
namespace EVA
{
	Ray Physics::ScreenPosToWorldRay(glm::ivec2 mousePos, const Camera* camera)
	{
		...
		const auto inverseProjectionMatrix = inverse(projectionMatrix);

		// The View Matrix goes from World Space to Camera Space.
		// So inverse(ViewMatrix) goes from Camera Space to World Space.
		const auto inverseViewMatrix = inverse(viewMatrix);

		// Faster way (just one inverse)
		const auto m = inverse(projectionMatrix * viewMatrix);
		...
		lRayDirWorld = normalize(lRayDirWorld);

		// Return the ray
		return {glm::vec3(lRayStartWorld), normalize(lRayDirWorld)};
	}

	bool Physics::TestRayObbIntersection(const Ray ray, const glm::vec3 aabbMin, const glm::vec3 aabbMax,
	                                     const glm::mat4 modelMatrix, float& intersectionDistance)
	{
		...
		// Test intersection with the 2 planes perpendicular to the OBB's X axis
		{
			...
			const auto e = dot(xaxis, delta);
			const auto f = dot(ray.direction, xaxis);
		...
		// Exactly the same thing than above.
		{
			...
			const auto e = dot(yaxis, delta);
			const auto f = dot(ray.direction, yaxis);
		...
		// Exactly the same thing than above.
		{
			...
			const auto e = dot(zaxis, delta);
			const auto f = dot(ray.direction, zaxis);
	...
	bool Physics::Raycast(const Ray ray, RaycastHit& out, Scene* scene)
	{
		...
		for (const auto& collider : scene->GetColliders())
		{
			...
			const auto rotationMatrix = toMat4(collider->transform->orientation);
			const auto translationMatrix = translate(glm::mat4(1.0f), collider->transform->position);
