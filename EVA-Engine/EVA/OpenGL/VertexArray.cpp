...
namespace EVA
{
	...
	void VertexArray::AddBuffer(const VertexBuffer& vb, const VertexBufferLayout& layout)
	{
		...
		for (auto element : elements)
		{
			...
			GLCall(glVertexAttribPointer(m_Attributes, element.count, element.type, element.normalized, layout.GetStride(), (
				const void*)offset));
	...
	void VertexArray::AddTempBuffer(const VertexBuffer& vb, const VertexBufferLayout& layout) const
	{
		...
		for (auto element : elements)
		{
			...
			GLCall(glVertexAttribPointer(attributeCount, element.count, element.type, element.normalized, layout.GetStride(), (
				const void*)offset));
			offset += element.count * VertexBufferElement::GetSizeOfType(element.type);
