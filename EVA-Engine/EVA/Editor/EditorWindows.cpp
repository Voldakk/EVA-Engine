...
namespace EVA
{
	...
	void EditorWindows::SceneHierarchy()
	{
		...
		ImGui::SetNextWindowSizeConstraints({300.0f, (float)screenSize.y - m_MenuBarHeight},
		                                    {(float)screenSize.x, (float)screenSize.y - m_MenuBarHeight});
	...
	void EditorWindows::AssetBrowser()
	{
		...
		// Folder tree
		ImGui::BeginChild(ImGui::GetID((void*)nullptr),
		                  ImVec2(200.0f, ImGui::GetWindowContentRegionMax().y - ImGui::GetWindowContentRegionMin().y), true);
		...
		// Current folder view
		ImGui::BeginChild(ImGui::GetID((void*)1), ImVec2(ImGui::GetWindowWidth() - 225.0f,
		                                                 ImGui::GetWindowContentRegionMax().y - ImGui::
		                                                 GetWindowContentRegionMin().y), true);
	...
	GameObject* EditorWindows::SelectedGameObject() const
	{
	...
	Light* EditorWindows::SelectedLight() const
	{
	...
	Skybox* EditorWindows::SelectedSkybox() const
	{
	...
	void EditorWindows::GameObjectInspector(const float width) const
	{
		...
		ImGui::InputFloat3("InspectorTransformPosition", value_ptr(position), "%.7g");

		gameObject->transform->SetPosition(position);

		ImGui::Text("Rotation");
		auto rotation = gameObject->transform->localRotation;
		if (ImGui::InputFloat3("InspectorTransformRotation", value_ptr(rotation), "%.7g"))
			gameObject->transform->SetOrientation(rotation);


		ImGui::Text("Scale");
		auto scale = gameObject->transform->localScale;
		ImGui::InputFloat3("InspectorTransformScale", value_ptr(scale), "%.7g");
	...
	void EditorWindows::RenameAssetPopup()
	{
		...
		if (ImGui::InputText("##NewName", m_NameBuffer, 10000, ImGuiInputTextFlags_EnterReturnsTrue))
		{
			...
			{
				std::cout << "EditorWindows::RenameAssetPopup - Failed to rename asset: '" << m_RenamePath << "' to '" << newPath <<
					"'" << std::endl
					<< e.what() << std::endl;
			}
