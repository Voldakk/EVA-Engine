...
namespace EVA
{
	Scene::Scene()
	{
		...
		m_ShadowMaterialInstanced.shader = ShaderManager::LoadShader(
			ShaderManager::STANDARD_SHADERS_PATH / "shadow_instaced.shader");

		m_ShadowMaterialCube.shader = ShaderManager::LoadShader(ShaderManager::STANDARD_SHADERS_PATH / "shadow_cube.shader");
		m_ShadowMaterialCubeInstanced.shader = ShaderManager::LoadShader(
			ShaderManager::STANDARD_SHADERS_PATH / "shadow_cube_instanced.shader");
	}
	...
	void Scene::RenderShadowCubeMap(const std::vector<glm::mat4>& shadowMatrices, const glm::vec3 lightPos,
	                                const float farPlane)
	{
		...
		for (auto& material : m_Materials)
		{
			...
			if (material[0][0]->material->useInstancing)
			{
				...
				{
					m_ShadowMaterialCubeInstanced.shader->SetUniformMatrix4Fv("shadowMatrices[" + std::to_string(i) + "]",
					                                                          shadowMatrices[i]);
				}
