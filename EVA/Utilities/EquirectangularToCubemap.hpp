...
namespace EVA
{
	...
	public:
		...
		inline static std::shared_ptr<Texture> Convert(int size, std::shared_ptr<Texture> hdrTexture)
		{
			FrameBufferRenderBuffer	fb(size);
			auto cubemap = TextureManager::CreateCubeMap(size, size, TextureFormat::RGB16F);
			...
			Material material;
			shader->Bind();
			shader->BindTexture(hdrTexture, "equirectangularMap");
